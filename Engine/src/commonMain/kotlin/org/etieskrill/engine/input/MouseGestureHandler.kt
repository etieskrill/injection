...
abstract class MouseGestureHandler(val primaryMouseKey: Key = Key.LEFT_MOUSE) : CursorInputHandler {
    ...
    override fun invokeClick(button: KeyEvent, posX: Double, posY: Double): Boolean {
        if (button.key == primaryMouseKey) {
            primaryMouseKeyDown = button.action != KeyEventAction.RELEASE
            prevX = posX
            prevY = posY
        }
        return false
