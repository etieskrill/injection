...
public class GLRenderer extends GLTextRenderer implements Renderer, TextRenderer, Disposable {
    ...
    private Vector4f clearColour = new Vector4f(CLEAR_COLOUR, CLEAR_COLOUR, CLEAR_COLOUR, 1);
    ...
    @ApplicationDisposed
    private Model box;
    private final Transform boxTransform = new Transform();
    ...
    private static Shaders.OutlineShader outlineShader;
...
//        _render(model, getOutlineShader(), combined);
    ...
    public void render(Pipeline<?> pipeline) {
        ...
        if (context.getActiveFramebuffer() != pipeline.getFrameBuffer()) {
...
//        }
    ...
    public void setClearColour(Vector4f clearColour) {
        this.clearColour = clearColour;
    }

    private boolean isBoxInitialised = false;

    public Model getBox() {
        if (!isBoxInitialised) {
            box = ModelFactory.box(new Vector3f(1f));
            isBoxInitialised = true;
        }
    ...
    private static boolean isOutlineShaderInitialised = false;

    public static Shaders.OutlineShader getOutlineShader() {
        if (!isOutlineShaderInitialised) {
            //TODO replace this trash with context calls
            outlineShader = (Shaders.OutlineShader) EngineShaderLoader.INSTANCE.load("outline", Shaders::getOutlineShader);
            isOutlineShaderInitialised = true;
        }
