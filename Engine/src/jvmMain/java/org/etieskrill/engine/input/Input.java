...
public class Input {
    ...
    public static final class BindPassOn {
        ...
        public BindPassOn group(OverruleGroup.Mode mode, KeyEvent... bindings) {
            Set<KeyEvent> binds = new HashSet<>(List.of(bindings));
        ...
        public BindPassOn group(OverruleGroup.Mode mode, Key... bindings) {
            return group(mode, Arrays.stream(bindings).map(Key::getInput).toArray(KeyEvent[]::new));
        }
