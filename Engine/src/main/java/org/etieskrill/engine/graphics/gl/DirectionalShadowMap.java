 ...
 * {@link DirectionShadowMap#texture} has the relevant comparator mode set up. The shadow sampler is slightly easier to use, and
 * creates better results out of the box.
 */
public class DirectionShadowMap extends FrameBuffer {

    private final Texture2D texture;

    protected DirectionShadowMap(Vector2ic size, Texture2D texture) {
        super(size);
        this.texture = texture;
    }

    public static DirectionShadowMap generate(Vector2ic size) {
        return new DirectionShadowMap.Builder(size).build();
    }
    ...
    public static class Builder extends FrameBuffer.Builder {
        ...
        @Override
        public DirectionShadowMap build() {
            GLUtils.clearError();
            DirectionShadowMap shadowMap = new DirectionShadowMap(size, texture);
