...
void main()
{
    ...
    //    vec3 chosenV0 = v1, chosenV1 = v2, chosenV2 = v0;
    vec3 chosenV0 = vec3(0.0), chosenV1 = vec3(0.0), chosenV2 = vec3(0.0);
    if (lenV0V1 <= lenV0V2 && lenV0V1 <= lenV1V2) {
        ...
        chosenV2 = v2;
        return;
    } else /*if (lenV1V2 <= lenV0V1 && lenV1V2 <= lenV0V2)*/ {
        chosenV0 = v0;
        chosenV1 = v2;
        chosenV2 = v1;
        //        return;
    ...
    if (length(chosenV0) == 0 || length(chosenV1) == 0 || length(chosenV2) == 0) return;
    ...
    for (int i = 1; i <= SUBDIVISIONS; i++) {
        gl_Position = mvp * vec4(v0 + d0 * i, 1.0);
        tTextureCoords = vec2(i / SUBDIVISIONS, i / SUBDIVISIONS);
        tFragPos = vec3(uModel * gl_Position);
        EmitVertex();
        gl_Position = mvp * vec4(v0 + d1 * i, 1.0);
