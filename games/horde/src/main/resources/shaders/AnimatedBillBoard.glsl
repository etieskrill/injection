...
void main()
{
    vec4 quadOffset = camera.projection * vec4(animatedBillBoard.size, 1, 1);
    quadOffset.xyz /= quadOffset.w;
    quadOffset.x = -quadOffset.x;

    vec4 pos = camera.combined * vec4(position + animatedBillBoard.offset, 1);
    pos.y -= quadOffset.y;
    ...
    for (int i = 0; i < 4; i++) {
        vec4 cornerOffset = camera.projection * vec4(rotation * (animatedBillBoard.size * corners[i]), 1, 1);
        ...
        vec2 lightCornerOffset = animatedBillBoard.size.xy * corners[i];
...
void main() {
    fragColour = texture(animatedBillBoard.sprite, vec3(texCoords, animatedBillBoard.layer));
    ...
    if (animatedBillBoard.punchThrough) {
