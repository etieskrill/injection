...
void main()
{
    ...
    for (int i = 0; i < NR_DIRECTIONAL_LIGHTS; i++) {
        vec4 dirLight = getDirLight(globalLights[i], normal, vertex.fragPos, viewPosition, 1);
        combinedLight += dirLight;
    }
    for (int i = 0; i < NR_POINT_LIGHTS; i++) {
        vec4 pointLight = getPointLight(lights[i], normal, vertex.fragPos, viewPosition, 1);
