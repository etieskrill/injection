...
class PlayerShipControllerService(private val window: Window) : Service {
    ...
    override fun process(
        ...
        if (controller.isPressed(Key.W)) inputDirection.direction.y += 1
        if (controller.isPressed(Key.S)) inputDirection.direction.y -= 1

        if (controller.isPressed(Key.A)) inputDirection.direction.x -= 1
        if (controller.isPressed(Key.D)) inputDirection.direction.x += 1
