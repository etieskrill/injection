...
object Cloth : App(
    ...
    val mainSphere = entitySystem.createEntity {
        +Transform()
        +Drawable(model("sphere") { sphere(1f, 1000) })
        +StaticCollider(Spheref(0f, 0f, 0f, 1f))
    }
    ...
    init {
        ...
        for (y in 0..<POINTS_DIM) {
            for (x in 0..<POINTS_DIM) {
                ...
                val entity = entitySystem.createEntity {
                    ...
                    +Drawable(clothPointModel)
                    +DynamicCollider(Spheref(0f, 0f, 0f, 0.01f), position)
                }
        ...
        check(clothPoints.all { it.hasComponents<MeshNeighbours>() }) {
            clothPoints.filter { !it.hasComponents<MeshNeighbours>() }.joinToString("\n")
        }

        entitySystem.addServices(
            RenderService(window.screenBuffer, renderer, camera, window.size).apply {
                postEffectsFrameBuffer.clearColour.set(0.05f, 0.05f, 0.05f, 1f)
                blur = false
            }
        )
    ...
    override fun render() {
        gridShader.setUniform("position", Vector3f(0f))
        gridShader.setUniform("camera", camera)

        TODO("cloth rendering")
//        gridShader.start()
