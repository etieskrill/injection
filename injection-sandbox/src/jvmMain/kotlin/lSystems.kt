...
object VertexAccessor : VertexArrayAccessor<Vertex>() {
    override fun registerFields() {
        addField<Vector2fc> { vertex, buffer -> buffer += vertex.position }
    }
}
...
class LSystemDemo(lines: List<Pair<Vector2fc, Vector2fc>>) : App() {
    init {
        window.size = Vector2i(1000)
    }

    val buffer = FrameBuffer.getColour(Vector2i(600))
    val vao = VertexArrayObject(
        VertexAccessor,
        lines.flatMap { listOf(Vertex(it.first), Vertex(it.second)) }.let { linePoints ->
            BufferObject(VertexAccessor, linePoints.size).apply { setData(linePoints) }
        },
        null,
    )
    val pipeline = Pipeline(
        ...
        DirectLineShader().apply {
            ...
            combined = OrthographicCamera(window.size).combined
        },
    ...
    override fun loop(delta: Double) {
        pipeline.shader.combined = OrthographicCamera(window.size).apply {
