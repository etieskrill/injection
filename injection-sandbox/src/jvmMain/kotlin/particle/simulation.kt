...
object FireSim : App() {
    ...
    override fun init() {
        ...
        entitySystem.addService(RenderService(window.screenBuffer, renderer, camera, window.size).apply { blur = false })

        entitySystem.createEntity {
            +Transform()
            +ParticleNode(listOf(emitter))
        }
    }
    ...
    override fun loop(delta: Double) {
        ...
        if (pacer.timerTime - lastTime - 100.milliseconds > 0.milliseconds) {
            container.spawn()
